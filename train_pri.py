...
from src.models.generator import Decoder, Discriminator, gan_loss, sync_Discriminator, PriorGrad
from learner import PriorGradLearner
import os
...
def train_net(args):
    ...
    g_params = [{'params': v_front.parameters()}, {'params': gen.parameters()}, {'params': prior.parameters()}]
    ...
    _ = validate(v_front, gen, prior, fast_validate=True)
    train(v_front, gen, prior, dis1, dis2, dis3, s_dis, train_data, args.epochs, optimizer=[g_optimizer, d_optimizer], scheduler=[g_scheduler, d_scheduler], args=args)

def train(v_front, gen, prior, dis1, dis2, dis3, s_dis, train_data, epochs, optimizer, scheduler, args):
    best_val_stoi = 0
    ...
    gen.train()
    Decoder().train()
    learner = PriorGradLearner(args.model_dir, prior, dataset, dataset_val, opt, params, fp16=args.fp16)
